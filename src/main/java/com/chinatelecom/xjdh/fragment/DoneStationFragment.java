...
import com.chinatelecom.xjdh.ui.DoneActivity_;
...
public class DoneStationFragment extends Fragment{
	...
	void getUserInfo() {
		try {
			...
			if (mApiResps.getRet() == 0) {
				...
				getData(mUserInfo.getId(),TYPE,LOCATYPE);
				return;
			}
		} catch (Exception e) {
			L.e(e.toString());
	...
	void getData(int user_id,int type,int locatype) {
		try {
			...
			if (apiResp.getRet() == 0) {
				...
				List<Country> list = Arrays.asList(apiResponseStationList.getSubsDevList());
				stList.addAll(list);
				if (apiResp.getRet() == 0) {
	...
	void stationGroupingClicked(int position) {
		DoneActivity_.intent(this).station_code(stList.get(position).getCountry_code()).start();
	}
