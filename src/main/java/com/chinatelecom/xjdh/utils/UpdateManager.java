...
public class UpdateManager {
	...
	public void checkAppUpdate(Context context, final boolean isShowMsg) {
		if (!AppContext_.getInstance().isNetworkConnected()) {
			T.showLong(context, "您的网络好像出问题了，请检测网络后重试。");
			return;
		}
	...
	private Runnable mdownApkRunnable = new Runnable() {
		...
		public void run() {
			try {
				String apkName = "xjdh_" + mUpdate.getVersionName() + ".apk";
				String tmpApk = "xjdh_" + mUpdate.getVersionName() + ".tmp";
				// 判断是否挂载了SD卡
				String storageState = Environment.getExternalStorageState();
				if (storageState.equals(Environment.MEDIA_MOUNTED)) {
					savePath = Environment.getExternalStorageDirectory().getAbsolutePath() + "/chinatelecom-xjdh/Update/";
